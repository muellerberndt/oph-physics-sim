"""Extract the E1 rich-fibre payload from the second preregistered 64k run.

Declared extraction step of
``docs/E1_PREREGISTERED_RICH_FIBRE_RUN_2_2026-08-10.md``.  Two declared
changes relative to the closed negative first preregistration
(``extract_e1_rich_fibre_payload.py``): ``support_truncation = 20``, and
the observer rule is greedy-disjoint selection: scan ``patch_observer``
rows of ``observer_views.jsonl`` in file order and select an observer
exactly when its first-20 support window is disjoint from every
previously selected window, stopping at four observers.  The selection
uses support node identities only; no record or companion datum enters
it.  Both changes are calibrated on the closed pilot's diagnostics and
declared before the fresh-seed run.
...
PAYLOAD_SCHEMA = "oph.sim.e1_rich_fibre_payload.v2"
MAX_PAYLOAD_BYTES = 200_000

DEFAULT_RUN_DIR = REPO_ROOT / "runs" / "e1_prereg2_64k_20260810"
DEFAULT_OUTPUT = REPO_ROOT / "docs" / "E1_RICH_FIBRE_PAYLOAD_2.json"
...
def _select_observers(
    views_path: Path, count: int, truncation: int
) -> list[dict]:
    """Greedy-disjoint selection in file order.

    Scan ``patch_observer`` rows and select a row exactly when its
    first-``truncation`` support window is disjoint from every
    previously selected window.  Support node identities only; no
    record or companion datum enters the selection."""
    ...
    with views_path.open() as handle:
        for line in handle:
            ...
            if row.get("view_type") != "patch_observer":
                continue
            scanned += 1
            window = [int(node) for node in row["support_nodes"][:truncation]]
            if len(window) < truncation:
                continue
            if used.isdisjoint(window):
                row["_scanned_rank"] = scanned
                selected.append(row)
                used.update(window)
            if len(selected) == count:
                return selected
    raise ValueError(
        f"observer_views yields only {len(selected)} pairwise-disjoint "
        f"patch observers at truncation {truncation}; {count} requested"
    )
...
def build_payload(run_dir: Path, *, support_truncation: int) -> dict:
    ...
    observers_raw = _select_observers(
        views_path, OBSERVER_COUNT, support_truncation
    )
    observers = []
        ...
        observers.append(
            {
                ...
                "file_order_scan_rank": row.get("_scanned_rank"),
                    ...
                    "extraction": OrderedDict(
                        {
                            ...
                            "observer_rule": (
                                "greedy-disjoint: scan patch_observer rows "
                                "of observer_views.jsonl in file order and "
                                "select a row exactly when its first-20 "
                                "window is disjoint from every previously "
                                "selected window, stopping at four; support "

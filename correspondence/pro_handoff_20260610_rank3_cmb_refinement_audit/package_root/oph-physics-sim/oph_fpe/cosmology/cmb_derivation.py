...
def _run_derivation_row(run_path: Path, targets: dict[str, Any]) -> dict[str, Any]:
    ...
    scalar_quotient = _read_json(run_path / "scalar_quotient_report.json")
    ...
    scalar_edge = scalar_quotient.get("edge_center_readout", {}) if isinstance(scalar_quotient, dict) else {}
    scalar_active = scalar_quotient.get("active_angular_levels", {}) if isinstance(scalar_quotient, dict) else {}
    scalar_gates = scalar_quotient.get("readiness_gates", {}) if isinstance(scalar_quotient, dict) else {}
    scalar_theta = _float_or_none(scalar_edge.get("theta_OPH_P_over_48"))
    scalar_ns = _float_or_none(scalar_edge.get("n_s_P_over_48"))
    if best_eta is None and scalar_theta is not None:
        best_eta = scalar_theta
    if best_ns is None and scalar_ns is not None:
        best_ns = scalar_ns
    scalar_ns_target_error = (
        abs(scalar_ns - float(targets["scalar_tilt"]["n_s"])) if scalar_ns is not None else None
    )
    ...
    run_gates = {
        "tilt_eta_R_simulator_compatible": bool(tilt_count > 0 or scalar_ns is not None),
        "scalar_quotient_receipt": bool(
            isinstance(scalar_quotient, dict) and scalar_quotient.get("SCALAR_QUOTIENT_RECEIPT", False)
        ),
        "scalar_quotient_active_33_level_freezeout_clause": bool(
            scalar_gates.get("active_33_level_freezeout_clause", False)
        ),
        "scalar_quotient_theorem_grade_release_code": bool(
            scalar_gates.get("theorem_grade_scalar_release_code", False)
        ),
        "scalar_quotient_finite_ready": bool(
            scalar_quotient.get("finite_lattice_cmb_scalar_release_ready", False)
        )
        if isinstance(scalar_quotient, dict)
        else False,
    ...
    return {
        ...
        "has_cmb_derivation_inputs": bool(
            scalar_quotient or anomaly or collar or parent_collar_ladder or cmb_stress or cl
        ),
        ...
        "n_s_abs_error": abs(best_ns - float(targets["scalar_tilt"]["n_s"])) if best_ns is not None else None,
        "scalar_quotient_receipt": run_gates["scalar_quotient_receipt"],
        "scalar_quotient_observer_count": _int_or_none(scalar_quotient.get("observer_count"))
        if isinstance(scalar_quotient, dict)
        else None,
        "scalar_quotient_patch_count": _int_or_none(scalar_quotient.get("patch_count"))
        if isinstance(scalar_quotient, dict)
        else None,
        "scalar_quotient_packet_alphabet_size": _int_or_none(scalar_quotient.get("scalar_packet_alphabet_size"))
        if isinstance(scalar_quotient, dict)
        else None,
        "scalar_quotient_packet_entropy_bits": _float_or_none(scalar_quotient.get("scalar_packet_entropy_bits"))
        if isinstance(scalar_quotient, dict)
        else None,
        "scalar_quotient_theta_OPH": scalar_theta,
        "scalar_quotient_n_s": scalar_ns,
        "scalar_quotient_n_s_target_abs_error": scalar_ns_target_error,
        "scalar_quotient_target_ell_IR": _int_or_none(scalar_active.get("target_ell_IR")),
        "scalar_quotient_observer_level_proxy": _int_or_none(
            scalar_active.get("observer_level_proxy_floor_sqrt_observers")
        ),
        "scalar_quotient_patch_capacity_level_proxy": _int_or_none(
            scalar_active.get("patch_capacity_level_proxy_floor_sqrt_patches")
        ),
        "scalar_quotient_active_33_level_freezeout_clause": run_gates[
            "scalar_quotient_active_33_level_freezeout_clause"
        ],
        "scalar_quotient_theorem_grade_release_code": run_gates["scalar_quotient_theorem_grade_release_code"],
        "scalar_quotient_finite_ready": run_gates["scalar_quotient_finite_ready"],
        "scalar_quotient_blockers": ";".join(str(item) for item in scalar_quotient.get("blockers", []))
        if isinstance(scalar_quotient, dict)
        else "",
...
def _aggregate(rows: list[dict[str, Any]], targets: dict[str, Any]) -> dict[str, Any]:
    ...
    scalar_theta_values = [
        float(row["scalar_quotient_theta_OPH"]) for row in rows if row.get("scalar_quotient_theta_OPH") is not None
    ]
    scalar_ns_values = [
        float(row["scalar_quotient_n_s"]) for row in rows if row.get("scalar_quotient_n_s") is not None
    ]
    scalar_entropy_values = [
        float(row["scalar_quotient_packet_entropy_bits"])
        for row in rows
        if row.get("scalar_quotient_packet_entropy_bits") is not None
    ]
    scalar_observer_values = [
        float(row["scalar_quotient_observer_count"])
        for row in rows
        if row.get("scalar_quotient_observer_count") is not None
    ]
    scalar_packet_values = [
        float(row["scalar_quotient_packet_alphabet_size"])
        for row in rows
        if row.get("scalar_quotient_packet_alphabet_size") is not None
    ]
    return {
        ...
        "mean_scalar_quotient_theta_OPH": fmean(scalar_theta_values) if scalar_theta_values else None,
        "mean_scalar_quotient_n_s": fmean(scalar_ns_values) if scalar_ns_values else None,
        "mean_scalar_quotient_n_s_abs_error": (
            fmean(abs(value - float(targets["scalar_tilt"]["n_s"])) for value in scalar_ns_values)
            if scalar_ns_values
            else None
        ),
        "mean_scalar_quotient_packet_entropy_bits": fmean(scalar_entropy_values) if scalar_entropy_values else None,
        "mean_scalar_quotient_observer_count": fmean(scalar_observer_values) if scalar_observer_values else None,
        "mean_scalar_quotient_packet_alphabet_size": fmean(scalar_packet_values) if scalar_packet_values else None,
        "run_count": len(rows),
...
def _markdown_report(report: dict[str, Any]) -> str:
    ...
    for key in (
        ...
        "mean_scalar_quotient_n_s_abs_error",
        "mean_scalar_quotient_packet_entropy_bits",
        "mean_scalar_quotient_observer_count",
        "mean_scalar_quotient_packet_alphabet_size",
    ):

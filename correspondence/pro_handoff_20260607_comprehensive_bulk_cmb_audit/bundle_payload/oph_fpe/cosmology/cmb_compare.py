    ...
    *,
    ...
    return {
        ...
        "claim_boundary": (
            ...
            "comparison is marked unusable unless the simulator covers the benchmark ell range; the "
            "overlap-ell comparison only scores the measured multipole interval covered by the finite "
            "screen. This is not a Planck likelihood, not CAMB/CLASS input, and not a physical CMB "
            "prediction."
        ),
    }
...
def _compare_spectrum_shape(
    ...
    *,
    ...
    overlap_ell = _overlap_ell_physical_comparison(sim, bench)
    return {
        "usable": True,
        "normalized_axis_shape_diagnostic": normalized_axis,
        "real_ell_physical_comparison": real_ell,
        "overlap_ell_physical_comparison": overlap_ell,
        "shape_correlation": corr,
...
def _overlap_ell_physical_comparison(
    sim: list[tuple[float, float]],
    bench: list[tuple[float, float]],
) -> dict[str, Any]:
    sim_ell = np.asarray([row[0] for row in sim], dtype=float)
    ...
    bench_min = float(np.min(bench_ell))
    bench_max = float(np.max(bench_ell))
    overlap_min = max(sim_min, bench_min)
    overlap_max = min(sim_max, bench_max)
    if overlap_max <= overlap_min:
        return {
            "usable": False,
            "reason": "no_ell_overlap",
            "sim_ell_min": sim_min,
            "sim_ell_max": sim_max,
            "benchmark_ell_min": bench_min,
            "benchmark_ell_max": bench_max,
            "claim_boundary": "finite screen has no real-ell overlap with the benchmark table",
        }
    mask = (bench_ell >= overlap_min) & (bench_ell <= overlap_max)
    overlap_count = int(np.sum(mask))
    if overlap_count < MIN_OVERLAP_BENCHMARK_BINS:
        return {
            "usable": False,
            "reason": "not_enough_overlap_benchmark_points",
            "overlap_ell_min": float(overlap_min),
            "overlap_ell_max": float(overlap_max),
            "overlap_benchmark_count": overlap_count,
            "min_overlap_benchmark_count": int(MIN_OVERLAP_BENCHMARK_BINS),
            "sim_ell_min": sim_min,
            "sim_ell_max": sim_max,
            "benchmark_ell_min": bench_min,
            "benchmark_ell_max": bench_max,
            "claim_boundary": "overlap exists but has too few benchmark bins for a shape diagnostic",
        }
    overlap_bench_ell = bench_ell[mask]
    overlap_bench_values = bench_values[mask]
    sim_interp = np.interp(overlap_bench_ell, sim_ell, sim_values)
    sim_z = _standardize(sim_interp)
    bench_z = _standardize(overlap_bench_values)
    amplitude = float(np.dot(sim_z, bench_z) / max(float(np.dot(sim_z, sim_z)), 1e-12))
    positive_amplitude = max(0.0, amplitude)
    residual = positive_amplitude * sim_z - bench_z
    corr = float(np.corrcoef(sim_z, bench_z)[0, 1]) if np.std(sim_z) > 1e-12 else 0.0
    return {
        ...
        "benchmark_ell_min": bench_min,
        "benchmark_ell_max": bench_max,
        "full_benchmark_covered": bool(sim_min <= bench_min and sim_max >= bench_max),
        "claim_boundary": (
            "real-ell overlap diagnostic only. It compares the benchmark multipole bins covered by the "
            "finite screen C_l proxy and remains a shape diagnostic, not a Planck likelihood or physical "
            "CMB prediction."
        ),
    }

...
def test_repair_clock_recovers_e_with_declared_time_normalization(tmp_path: Path) -> None:
    ...
    assert report["finite_repair_clock_certificate"] is True
    assert abs(report["summary"]["median_kappa_rep_estimate"] - math.e) < 1.0e-10
    assert abs(report["summary"]["median_eta_R_estimate"] - math.e * delta) < 1.0e-12
...
def _write_exponential_mismatch_trace(path: Path, *, delta: float, dt: float, amplitude: float) -> None:
    with path.open("w", newline="", encoding="utf-8") as handle:
        ...
        for cycle in range(24):
            phi = amplitude * math.exp(-math.e * delta * float(cycle) * float(dt))

...
def comparable_data_report(run_dirs: list[Path]) -> dict[str, Any]:
    ...
    strict_neutral_bulk_count = sum(1 for row in rows if bool(row.get("bulk_proof_strict_neutral_3d_bulk")))
...
def _extract_run_row(run_path: Path) -> dict[str, Any]:
    ...
    row = {
        ...
        "repair_scale_N_pred": _nested(repair_scale, "closure_outputs", "capacity_predicted_from_local_P"),
...
def _repair_scale_closure_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "interpretation": (
            ...
            "|g'(P)|=alpha/phi^2, an implied N_CRC=|g'|^-48, and n_s=1-P/48. It explains finite-regulator "
            "shallow depth, but does not derive the 24 rounds from a finite selector and does not establish "
            "populated 3D bulk or physical CMB."
        ),
    }
...
def _markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- mean predicted N_CRC from P: {_fmt(repair_scale['mean_N_pred'])}",
...
def _best_object_chart_report(root: Path) -> dict[str, Any]:
    ...
    def score(report: dict[str, Any]) -> tuple[float, float, float, float, float, float, float, float]:

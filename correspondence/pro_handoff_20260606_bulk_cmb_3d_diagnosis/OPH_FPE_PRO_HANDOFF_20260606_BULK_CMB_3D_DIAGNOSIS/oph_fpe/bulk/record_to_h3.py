...
def observer_chart_object_population_report(
    ...
    *,
    ...
    if str(incidence_mode) in {"observer_transition_mixture_cluster", "transition_history_mixture_cluster"}:
    ...
    localized_subpopulation_median_receipt = bool(
        localized_not_boundary_count >= int(min_localized_objects)
        and localized_not_boundary_count > shuffled_localized_count
        and bool(h3_report.get("MODULAR_RESPONSE_KERNEL_TO_H3_RECEIPT", False))
    )
    localized_subpopulation_receipt = bool(
        localized_not_boundary_count >= int(min_localized_objects)
        and (
            (
                shuffled_localized_p90 is not None
                and localized_not_boundary_count > float(shuffled_localized_p90)
            )
            or (
    ...
    return {
        ...
        "localized_object_median_precursor_receipt": localized_subpopulation_median_receipt,
        "localized_object_precursor_receipt": localized_subpopulation_receipt,
        ...
        "claim_boundary": (
            ...
            "particle claim. When split_h3_components is true, broad observer-visible packet classes are "
            "split into local H3 components and the same split is applied to shuffled controls."
        ),
    }

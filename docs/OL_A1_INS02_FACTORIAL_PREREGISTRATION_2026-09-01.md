...
conserved labels, or producer capabilities. This S1 freeze hashes this
preregistration, its template, and the five direct analysis paths named in the
manifest; it does not hash transitive imports, including
...
`oph_fpe/core/echosahedral_federation.py`, and
`oph_fpe/core/icosahedral.py`, nor the manifest or its schema. It is therefore
a pin of declared direct analysis paths, not a complete executable-dependency
closure. If stage S2 finds otherwise, the change goes through the upstream
premise register's recorded-decision process before any run, the recorded

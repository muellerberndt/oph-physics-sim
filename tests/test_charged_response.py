...
def test_derived_construction_and_signed_scales(artifact: dict) -> None:
    ...
    assert derived["construction_provenance"] == "derived_from_measured_sector_structure"
...
def test_measured_responses(artifact: dict) -> None:
    measured = artifact["measured_response"]
    assert measured["potential_response"]["unit_channel_constant"] == "10 + 2*sqrt(5)"
    assert measured["potential_response"]["response_sign"] == -1
    assert measured["rotation_response"]["incidence_automorphism_group_order"] == 120
    assert measured["rotation_response"]["commuting_incidence_automorphisms"] == 60
    assert measured["rotation_response"]["response_sign"] == 1

...
def test_derived_clock_candidates_carry_provenance_and_spectral_diagnostic(
    tmp_path: Path,
) -> None:
    run = tmp_path / "run"
    out = tmp_path / "out"
    ...
    assert all("claim_status" in c for c in candidates)
    assert all(c["theorem_bound_certified"] is False for c in candidates)
    # The matrix-derived value is explicitly a spectral-relaxation diagnostic,
    # never a claimed mixing time or a physical clock binding.
    relaxation = by_name["transition_matrix_spectral_relaxation_time"]
    assert relaxation["provenance"] == "matrix_derived"
    assert relaxation["derived_from_run_data"] is True
    assert relaxation["claim_status"] == "run_derived_diagnostic"
    assert relaxation["mixing_time_claimed"] is False
    assert relaxation["usable_for_physical_clock_binding"] is False
    assert relaxation["degenerate"] is False
    assert relaxation["value"] is not None and relaxation["value"] > 0.0
    ...
    assert provenance["usable_derived_candidate_count"] == 0
    assert provenance["theorem_bound_certified_count"] == 0


def test_spectral_relaxation_diagnostic_fails_closed_on_gapless_chain(
    tmp_path: Path,
) -> None:
    run = tmp_path / "run"
    out = tmp_path / "out"
    run.mkdir()
    # Periodic chain -> abs(lambda_2) == 1.0 -> no finite spectral-relaxation time.
    _write_observer_views(
        run / "observer_views.jsonl",
        [
            [0, 1, 0, 1],
    ...
    relaxation = next(
        ...
        if c["name"] == "transition_matrix_spectral_relaxation_time"
    )
    assert relaxation["degenerate"] is True
    assert relaxation["value"] is None
    assert relaxation["mixing_time_claimed"] is False
    assert "strictly between zero and one" in relaxation["reason"]
...
def test_mean_first_passage_time_candidate_reads_run_histogram(tmp_path: Path) -> None:
    ...
    assert fpt["claim_status"] == "run_derived_diagnostic"
    assert fpt["usable_for_physical_clock_binding"] is False
    assert fpt["theorem_bound_certified"] is False

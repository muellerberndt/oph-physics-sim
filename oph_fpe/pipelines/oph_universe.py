...
def _write_readme(path: Path, summary: dict[str, Any]) -> None:
    ...
    computed_receipts = {
        ...
        if key != "simulation_assumed_visual_universe_receipt"
    }
    ...
    for key, value in computed_receipts.items():
        lines.append(f"- `{key}`: `{str(bool(value)).lower()}`")
    lines.extend(
        [
            "",
            "## Visualization Assumption Marker",
            f"- `simulation_assumed_visual_universe_receipt`: "
            f"`{str(visual_assumption_receipt).lower()}`",
            f"- profile: `{assumption_manifest.get('profile')}`",
            f"- scope: `{assumption_manifest.get('scope')}`",
            "",
            "A true visualization-assumption marker means the declared explanatory scene profile is "
            "complete. It is not a computed proof receipt or a physical prediction.",
            "",
            "## Viewers",

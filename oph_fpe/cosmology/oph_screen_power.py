...
def write_oph_screen_power_report(
    ...
    *,
    ...
    reference_params, reference_source, simulator_reference_ready = _reference_params(rows)
...
def _reference_params(rows: list[dict[str, Any]]) -> tuple[OPHScreenPowerParams, str, bool]:
    available = [row for row in rows if row.get("fit_available")]
    if not available:

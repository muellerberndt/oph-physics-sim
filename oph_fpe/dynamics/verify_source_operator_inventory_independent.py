...
# This descendant pins the load quotient and therefore depends transitively on
# the inventory. It must be excluded from the inventory's raw-input hash set.
...
EXPECTED_CONTRACTS: dict[str, dict[str, Any]] = {
    ...
    "data/causal_order/source_derived_causal_order_receipt.json": _contract(
        "oph.source-derived-causal-order.v2",
...
def _rows(paths: Sequence[str]) -> list[dict[str, Any]]:
    ...
    for path in sorted(current):
        ...
        if path == "data/refinement/all_level_primitive_seam_source_receipt.json":
            ...
            if verification.get("receipt") is not True:
                raise ValueError(
                    "registered-ladder primitive seam source failed independent verification"
                )
        if path == "data/refinement/a5_biposh_continuum_tail_receipt.json":

...
def observer_chart_object_population_report(
    ...
    *,
    ...
    h3_margin_vs_median_shuffle = (
        float((median_shuffled - median_h3) / median_shuffled)
        if np.isfinite(median_h3) and np.isfinite(median_shuffled) and abs(median_shuffled) > 1e-12
        else None
    )
    h3_margin_vs_p10_shuffle = (
        float((float(p10_shuffled) - median_h3) / float(p10_shuffled))
        if (
            np.isfinite(median_h3)
            and p10_shuffled is not None
            and np.isfinite(float(p10_shuffled))
            and abs(float(p10_shuffled)) > 1e-12
        )
        else None
    )
    ...
    h3_margin_vs_s2_boundary = (
        float((median_s2 - median_h3) / median_s2)
        if np.isfinite(median_h3) and np.isfinite(median_s2) and abs(median_s2) > 1e-12
        else None
    )
    ...
    localized_fraction = float(localized_count / max(len(object_rows), 1))
    localized_not_boundary_fraction = float(localized_not_boundary_count / max(len(object_rows), 1))
    shuffled_localized_fraction = float(shuffled_localized_count / max(len(object_rows), 1))
    shuffled_localized_not_boundary_fraction = float(shuffled_localized_not_boundary_count / max(len(object_rows), 1))
    localized_count_saturated = bool(localized_fraction >= 0.98 and shuffled_localized_fraction >= 0.98)
    localized_not_boundary_count_saturated = bool(
        localized_not_boundary_fraction >= 0.98 and shuffled_localized_not_boundary_fraction >= 0.98
    )
    ...
    compactness_distribution_control_receipt = bool(
        eligible
        and chart_receipt
        and h3_localized
    ...
    return {
        ...
        "h3_compactness_margin_vs_median_shuffle": h3_margin_vs_median_shuffle,
        "h3_compactness_margin_vs_p10_shuffle": h3_margin_vs_p10_shuffle,
        "h3_compactness_margin_vs_s2_boundary": h3_margin_vs_s2_boundary,
        "localized_object_count": localized_count,
        "localized_not_boundary_object_count": localized_not_boundary_count,
        "localized_object_fraction": localized_fraction,
        "localized_not_boundary_object_fraction": localized_not_boundary_fraction,
        ...
        "shuffled_localized_object_fraction": shuffled_localized_fraction,
        "shuffled_localized_not_boundary_object_fraction": shuffled_localized_not_boundary_fraction,
        "localized_count_saturation_warning": localized_count_saturated,
        "localized_not_boundary_count_saturation_warning": localized_not_boundary_count_saturated,
        ...
        "compactness_distribution_control_receipt": compactness_distribution_control_receipt,
        ...
        "observer_facing_h3_object_population_receipt": strict_bulk_population_receipt,

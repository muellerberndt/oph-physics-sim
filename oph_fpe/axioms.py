...
def _a1_realization(config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "axiom": "A1",
        "realized_fragment": (
            "Finite observer patch net on an oriented spherical support with "
            "twelve-port echosahedral carriers, typed seams, records, repair "
        ...
        "realization_status": (
            "architectural fragment; the faithful compact response object of "
            "the strengthened A1 source clause is carried by the theory "
...
def _a2_realization(config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "realization_status": (
            ...
            "overlap data, not proved for all recharting maps"
        ),
    }
...
def _a3_realization(config: dict[str, Any]) -> dict[str, Any]:
    return {
        "axiom": "A3",
        "realized_fragment": (
            "Conditional maximum randomness realized as the exact "
            "conditional-resampling kernel on observation fibers: everything "
            "the protected record datum leaves unconstrained is resampled "
            "from the pinned reference law restricted to the fiber."
        ),
        "simulator_structures": {
        ...
        "realization_status": (
            ...
            "with a pinned common reference; the full information-projection "
            "clause over the complete constraint set is theory-side"
        ),
    }
